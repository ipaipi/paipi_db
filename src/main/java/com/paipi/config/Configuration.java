...
public class Configuration {
    ...
    private Configuration(final String json) {
        try {
            ...
            throw new DbAdapterException(String.format("配置信息错误. 您提供的配置信息不是合法的JSON格式: %s . 请按照标准json格式提供配置信息. ", e.getMessage()));
        }
    }
    ...
    public static String replaceVariable(final String param) {
        ...
        Matcher matcher = Pattern.compile("(\\$)\\{?(\\w+)\\}?").matcher(param);
        while (matcher.find()) {
    ...
    public static Configuration from(File file) {
        try {
            ...
            throw new DbAdapterException(String.format("配置信息错误，您提供的配置文件[%s]不存在. 请检查您的配置文件.", file.getAbsolutePath()));
        } catch (IOException e) {
            throw new DbAdapterException(String.format("配置信息错误. 您提供配置文件[%s]读取失败，错误原因: %s. 请检查您的配置文件的权限设置.", file.getAbsolutePath(), e));
        }
    }
    ...
    public static Configuration from(InputStream is) {
        try {
            ...
            throw new DbAdapterException(String.format("请检查您的配置文件. 您提供的配置文件读取失败，错误原因: %s. 请检查您的配置文件的权限设置.", e));
        }
    }
    ...
    private static void checkJSON(final String json) {
        if (StringUtils.isBlank(json)) {
            throw new DbAdapterException("配置信息错误. 因为您提供的配置信息不是合法的JSON格式, JSON不能为空白. 请按照标准json格式提供配置信息. ");
        }
    }
    ...
    public String getNecessaryValue(String key) {
        ...
        if (StringUtils.isBlank(value)) {
            throw new DbAdapterException(String.format("您提供配置文件有误，[%s]是必填参数，不允许为空或者留白 .", key));
        }
    ...
    public Character getChar(final String path) {
        ...
        try {
            ...
            throw new DbAdapterException(String.format("任务读取配置文件出错. 因为配置文件路径[%s] 值非法，期望是字符类型: %s. 请检查您的配置并作出修改.", path, e.getMessage()));
        }
    }
    ...
    public Boolean getBool(final String path) {
        ...
        if (null == result) {
            ...
            throw new DbAdapterException(String.format("您提供的配置信息有误，因为从[%s]获取的值[%s]无法转换为bool类型. 请检查源表的配置并且做出相应的修改.", path, result));
        }
    ...
    public Integer getInt(final String path) {
        ...
        try {
            ...
            throw new DbAdapterException(String.format("任务读取配置文件出错. 配置文件路径[%s] 值非法, 期望是整数类型: %s. 请检查您的配置并作出修改.", path, e.getMessage()));
        }
    }
    ...
    public Double getDouble(final String path) {
        ...
        try {
            ...
            throw new DbAdapterException(String.format("任务读取配置文件出错. 配置文件路径[%s] 值非法, 期望是浮点类型: %s. 请检查您的配置并作出修改.", path, e.getMessage()));
        }
    }
    ...
    public String beautify() {
        return JSON.toJSONString(this.getInternal(), SerializerFeature.PrettyFormat, SerializerFeature.WriteMapNullValue, SerializerFeature.WriteDateUseDateFormat);
    }
    ...
    public Object remove(final String path) {
        ...
        if (null == result) {
            throw new DbAdapterException(String.format("配置文件对应Key[%s]并不存在，该情况是代码编程错误. 请联系DataX团队的同学.", path));
        }
    ...
    private void setObject(final String path, final Object object) {
        ...
        throw new DbAdapterException(String.format("值[%s]无法适配您提供[%s]， 该异常代表系统编程错误, 请联系DataX开发团队!", ToStringBuilder.reflectionToString(object), path));
    }
    ...
    Object buildObject(final List<String> paths, final Object object) {
        if (null == paths) {
            throw new DbAdapterException("Path不能为null，该异常代表系统编程错误, 请联系DataX开发团队 !");
        }
        ...
        for (int i = paths.size() - 1; i >= 0; i--) {
            ...
            throw new DbAdapterException(String.format("路径[%s]出现非法值类型[%s]，该异常代表系统编程错误, 请联系DataX开发团队! .", StringUtils.join(paths, "."), path));
        }
    ...
    Object setObjectRecursive(Object current, final List<String> paths, int index, final Object value) {
        ...
        throw new DbAdapterException("该异常代表系统编程错误, 请联系DataX开发团队 !");
    }
